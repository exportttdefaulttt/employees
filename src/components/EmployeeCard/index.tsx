...
import type { Employee, EmployeeBooleanKeys } from '@/types/employee';

import './EmployeeCard.scss';

interface EmployeeCardProps extends Employee {
    onRemove: (id: string) => void;
    onToggle: (id: string, prop: EmployeeBooleanKeys) => void;
    onOpenModal: (id: string) => void;
}
const EmployeeCard = ({ 
    id, firstName, lastName, email, position, salary, 
    isBonus, isImportant, createdAt, updatedAt, onRemove, 
        ...
        <article className={clsx('employee-card', isBonus && 'employee-card--bonus', isImportant && 'employee-card--important')}>
            ...
            <div className="employee-side">
                ...
                <div className="employee-actions">
                    ...
                    <button
                        onClick={() => onToggle(id, 'isBonus')}
                        type="button"
                        className="reward"
                        aria-label="Выдать премию"
                    >
                        <Award size={18} />
                    </button>

                    <button
                        onClick={() => onToggle(id, 'isImportant')}
                        type="button"
                        className="star"
                        aria-label="Важный сотрудник"
                    >
                        <Star size={18} />
                    </button>
